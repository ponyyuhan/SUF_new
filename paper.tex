...
\begin{itemize}[nosep,leftmargin=*]
 \item \textbf{State-of-the-art secure inference performance.}
       FuseFSS improves end-to-end latency and preprocessing cost over state-of-the-art two-server GPU secure inference baselines, while matching model accuracy on BERT and GPT. Upon acceptance.
...
We compute over $R=\mathbb{Z}_{2^n}$. For $x\in R$, let $\operatorname{rep}(x)\in\{0,\dots,2^n-1\}$ denote its canonical representative. Unsigned comparisons interpret each element by $\operatorname{rep}(x)$. Signed values use two's complement; the most significant bit (MSB) is $\msb(x):=\one[\operatorname{rep}(x)\in[2^{n-1},2^n)]$. We write $\bits:=\{0,1\}$ and $\one[\mathcal{E}]\in\bits$ for the indicator of a predicate/event $\mathcal{E}$.
...
\noindent\textbf{Low-bit predicates.} An analogous identity holds for low-bit predicates of the form $\one[(x\bmod 2^f)<\gamma]$; we defer the full statement to Lemma~\ref{lem:mask_rewrite_lowbit} in Appendix~\ref{app:lowbit_rewrite}.
...
Correctness follows from compiler correctness for the operator specification and the correctness of the post-processing circuit.
Security follows from the single-key security of the two FSS primitives and the security of the preprocessing-based subprotocols used in post-processing.
We state this formally in Theorem~\ref{thm:gate_security_leakage}.
...
We report per-gate online evaluation time and online communication, together with the preprocessing key material size.
...
\If{$\beta_0 = 0$} \Comment{$0\in\{s_i\}$, i.e., the wrap point hits an existing boundary; translated partition has $m$ intervals}
  \State $B \gets (0,\beta_1,\ldots,\beta_{m-1},N)$
  \State $\mathrm{ord} \gets (\pi(0),\pi(1),\ldots,\pi(m-1))$
  \If{$M = m+1$} \Comment{equivalently $m < N$; pad by splitting one standard interval}
    \State Let $j^\star$ be the \emph{smallest} index such that $B_{j^\star+1}-B_{j^\star}\ge 2$ \Comment{exists since $m<N$}
    \State $\delta \gets B_{j^\star}+1$ \Comment{any integer with $B_{j^\star}<\delta<B_{j^\star+1}$ works}
    \State Insert $\delta$ into $B$ \emph{at position} $j^\star{+}1$ \Comment{split $[B_{j^\star},B_{j^\star+1})$}
    \State Insert a copy of $\mathrm{ord}_{j^\star}$ into $\mathrm{ord}$ \emph{at position} $j^\star{+}1$
    \Comment{duplicate payload so both sub-intervals return identical data}
  \EndIf
\Else \Comment{$0\notin\{s_i\}$; exactly one translated interval wraps around $0$ and is split at $0$ (thus $m<N$ and $M=m+1$)}
  \State $B \gets (0,\beta_0,\beta_1,\ldots,\beta_{m-1},N)$
  \State $\mathrm{ord} \gets (\pi(m-1),\pi(0),\pi(1),\ldots,\pi(m-1))$
  \Comment{$\pi(m-1)$ duplicated for the two pieces $[0,\beta_0)$ and $[\beta_{m-1},N)$}
\EndIf
...
\]
A backend provides algorithms $(\mathsf{Gen},\mathsf{Eval}_0,\mathsf{Eval}_1)$
...
A backend provides algorithms $(\mathsf{Gen},\mathsf{Eval}_0,\mathsf{Eval}_1)$ such that $\mathsf{Gen}(1^\lambda,(\{\alpha_i\},\{v_i\}))\to(k_0,k_1)$ and,
...
if it coincides with an existing boundary (so that only $m$ standard intervals arise), inserting one additional dummy split point $\delta$ strictly inside a standard interval and duplicating that interval's payload on both sides. (Both sub-intervals may be selected, but they return identical payloads, so correctness is preserved.) This yields a fixed $M=m+1$.
Finally, $p=r(d+1)+p_{\mathrm{aux}}$ is fixed by polynomial degree and auxiliary
constants required by post-processing, hence determined by the operator specification and metadata.
\end{proof}
...
Online evaluation will open the public masked value $\hatx = x + \rin \bmod N$ using Algorithm~\ref{alg:shares_to_masked}.

\emph{Interval lookup instance $\Pi_{\mathrm{coeff}}$.}
There are $m=2$ original intervals, so we allocate $M=m+1=3$ intervals after padding (Protocol~\ref{alg:compile_tfss}).
...
Following Algorithm~\ref{alg:compile_tfss}, we construct boundaries $B=(B_0,\ldots,B_M)$ and an index list $\mathrm{ord}\in\{0,1\}^M$ that may duplicate one payload so that the \emph{public} lookup shape $(M,p)$ is independent of the sampled mask.
...
Let $T$ be the emitted number of primitive masked comparison atoms in $\Pi_{\mathrm{pred}}$ after any specification-only syntactic deduplication and optional padding, so that $T$ depends only on the public operator specification and metadata.
Let $M$ and $p$ be the interval count and payload dimension of $\Pi_{\mathrm{coeff}}$ under the padding convention, where $M=m+1$ for an operator specification with $m$ original intervals.
Let $M_{\mathrm{A}}$ be the number of ring multiplications for Horner evaluation and post-processing, and let $M_{\mathrm{B}}$ be the number of Boolean ANDs.
